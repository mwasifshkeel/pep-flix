...
export default function MovieCard(props) {
  ...
  const removeFromQueue = () => {
    const newQueueItems = queueItems.filter(
      (queueMovie) => queueMovie.title !== movie.title
    );
  ...
  const getCountryName = (code) => {
    ...
    return countryMap[code] || "Unknown Country";
  };
  ...
  return (
    <div className="relative h-[430px] rounded-2xl flex flex-col max-w-[400px] gap-3 sm:gap-6 bg-zinc-600 w-full p-3 text-slate-100">
      ...
      <div>
        <h3 className="text-sm sm:text-base uppercase">
          Country:{" "}
          <span className="text-md sm:text-lg text-gray-400">
            {getCountryName(movie.original_language)}
          </span>
        </h3>
        <h3 className="text-sm sm:text-base uppercase">
          Language:{" "}
      ...
      <div className="flex-1 overflow-hidden">
        <div
          className={`plot-container ${
            movie.overview.length > thresholdLength ? "scrollable" : ""
          }`}
        >
          <p className="text-base sm:text-md text-gray-400">{movie.overview}</p>
        </div>
      </div>
        ...
        <div className="flex gap-2 flex-wrap text-xs sm:text-sm">
          {movie.genre_ids.map((genreId, genreIndex) => {
            ...
            return (
              <div
                key={genreIndex}
                className="pointer-events-none select-none rounded-2xl flex items-center justify-center bg-zinc-800 text-gray-300 w-[90px] h-[40px] border-amber-200 border-[1px]"
              >
                <p>{genreName}</p>
              </div>
            );
      ...
      <div className="absolute top-0 right-0 translate-x-2 -translate-y-2 opacity-30">
        <button
          onClick={
            queueItems.some((item) => item.title === movie.title)
              ? removeFromQueue
              : addToQueue
          }
        >
          <i
            className={`fa-solid text-2xl hover:text-slate-400 ${
              queueItems.some((item) => item.title === movie.title)
                ? "fa-circle-minus addqueue"
                : "fa-circle-plus removequeue"
            }`}
            data-tooltip-id="queue-status"
            data-tooltip-content={
              queueItems.indexOf(movie) !== -1
                ? "Remove from queue"
                : "Add to queue"
            }
          />
